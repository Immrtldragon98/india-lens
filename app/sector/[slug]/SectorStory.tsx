...
export default function SectorStory({sector}:{sector:any}){
 ...
 return <section className="sectorStory">
  ...
  <div className="sectorJourney"><small>CONTINUE THE SAME STORY</small><h3>{journey.shock}</h3><div className="journeyChain">{journey.chain.map((x:string,i:number)=><span key={x}><b>{i+1}</b>{x}{i<journey.chain.length-1&&<i>→</i>}</span>)}</div><div className="journeyActions"><Link href={"/economy?ask="+encodeURIComponent("Explain how "+journey.shock+" affects India and guide me through an experiment")}>← Test the macro shock in Economy Lab</Link><span>{journey.industry}</span>{journey.symbol?<Link href={"/market-lens?symbol="+encodeURIComponent(journey.symbol)+"&from="+sector.slug+"&question="+encodeURIComponent("Help me understand "+journey.company+" using GAJA, starting with what the business actually does")}>Study {journey.company} with GAJA →</Link>:<Link href="/market-lens">Choose a company in Market Lens →</Link>}</div></div>
 </section>
}
